...
class ApiController extends Controller
{
    ...
    public function accessTokenXhr(Request $request)
    {
        $json = [];
        $user = $this->getUser();
        if($user && $request->isXmlHttpRequest()) {
            $req = $request->request->all()?:json_decode($request->getContent(),true);
            $data = $this->container->get('fos_oauth.access_token_manager');
            ...
            if($request->getMethod() == 'GET') {
                ...
                if(!$client) {
                    ...
                    // $clientManager = $this->conatiner->get('fos_oauth.client_manager');
                    // $client = $clientManager->createClient();
                    // $client->setRedirectUris(['/']);
                    // $client->setAllowedGrantTypes(["authorization_code", "password", "refresh_token"]);
                    // $client->setUser($user? : null);
                ...
                if(isset($req['name']) && ($reqName = $req['name'])) {
                    ...
                    $json['alertMessage'] = "Success! access token added successfully";
                    $json['id'] = $token->getId();
                    $json['expiresAt'] = $token->getExpiresAt();
                    $json['token'] = $token->getToken();
                } else {
                    $json['alertClass'] = 'danger';
                    $json['alertMessage'] = "Error! provide a token name.";
                ...
                if(isset($id)) {
                    ...
                    if($token) {
                        ...
                        $json['alertClass'] = 'success';
                        $json['alertMessage'] = "Success! access token deleted successfully";
                    } else {
                        $error = true;
                    }
                } else {
                ...
                if(isset($req['name']) && ($name = $req['name'])) {
                    ...
                    if($token) {
                        ...
                        $data->updateToken($token);
                        $json['alertClass'] = 'success';
                        $json['alertMessage'] = "Success! access token updated successfully";
                    } else {
                        $json['alertClass'] = 'danger';
                        $json['alertMessage'] = "Error! invalid token";    
                        $statusCode = Response::HTTP_BAD_REQUEST;
                    }
                } else {
                    $json['alertClass'] = "danger";
                ...
                if(isset($req['token'])) {
                    ...
                    if($token) {
                        ...
                        $json['expiresAt'] = $token->getExpiresAt();
                        $json['alertClass'] = 'success';
                        $json['alertMessage'] = "Success! access token refreshed successfully";
                    } else {
                        $error = true;
                    }
                } else {
